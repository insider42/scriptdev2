...
{"Вы готовы драться с чемпионом Серебряного Рассвета?",GOSSIP_ACTION_INFO_DEF+2,false,TYPE_ARGENT_CHAMPION},  //
...
switch(uiAction) {
    ...
    if ((m_pInstance->GetData(TYPE_ARGENT_CHAMPION) == NOT_STARTED || m_pInstance->GetData(TYPE_ARGENT_CHAMPION) == IN_PROGRESS) && m_pInstance->GetData(TYPE_GRAND_CHAMPIONS) == DONE)
                {
    ...
    if ((m_pInstance->GetData(TYPE_BLACK_KNIGHT) == NOT_STARTED || m_pInstance->GetData(TYPE_BLACK_KNIGHT) == IN_PROGRESS) && m_pInstance->GetData(TYPE_ARGENT_CHAMPION) == DONE)
            {
