...
struct MANGOS_DLL_DECL boss_devourer_of_soulsAI : public ScriptedAI
{
    ...
    void SpawnOutro(uint32 guid)
    {
        if (Creature *pSummon = m_creature->SummonCreature(guid, SpawnLoc[0].x, SpawnLoc[0].y, SpawnLoc[0].z,0, TEMPSUMMON_TIMED_DESPAWN, 300000))
        {
            ...
            if (pSummon->GetEntry() == NPC_JAINA_END)
                DoScriptText(SAY_JAINA_FS09_EXTRO, pSummon);
            else if (pSummon->GetEntry() == NPC_SILVANA_END)
    ...
    void JustDied(Unit* pKiller)
    {
       ...
       if(player->GetTeam() == ALLIANCE)
       {
            ...
            SpawnOutro(NPC_JAINA_END);
            SpawnOutro(npc_jaina_credit);
            SpawnOutro(NPC_ARCHMAGE_KORELN);
            SpawnOutro(NPC_ARCHMAGE_ELANDRA);
            SpawnOutro(NPC_COLISEUM_CHAMPION_A_M);
            SpawnOutro(NPC_COLISEUM_CHAMPION_A_F);
            SpawnOutro(NPC_COLISEUM_CHAMPION_A_P);

       } else
       {
            m_pInstance->SetData64(DATA_LIDER,1);
            SpawnOutro(NPC_SILVANA_END);
            SpawnOutro(npc_sylvana_credit);
            SpawnOutro(NPC_ARCHMAGE_KORELN);
            SpawnOutro(NPC_ARCHMAGE_ELANDRA);
            SpawnOutro(NPC_COLISEUM_CHAMPION_H_M);
            SpawnOutro(NPC_COLISEUM_CHAMPION_H_F);
            SpawnOutro(NPC_COLISEUM_CHAMPION_H_T);
        };
    }
...
struct MANGOS_DLL_DECL npc_well_of_soulAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(m_pInstance && m_pInstance->GetData(TYPE_DECOURER_OF_SOULS) != IN_PROGRESS)
...
struct MANGOS_DLL_DECL npc_unleashed_soulAI : public ScriptedAI
{
    ...
    void Reset()
    {
        if (m_pInstance)
        {
            if (Creature* pDevourer = m_creature->GetMap()->GetCreature( m_pInstance->GetData64(NPC_DEVOURER_OF_SOULS)))
