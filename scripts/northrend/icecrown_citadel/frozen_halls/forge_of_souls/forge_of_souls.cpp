...
bool StartEvent;
    ...
    {
            if(m_pInstance) 
               if(m_pInstance->GetData(TYPE_DECOURER_OF_SOULS) == DONE)
...
uint32 uiSummon_counter;
    ...
    void Reset()
    {
            if (m_pInstance)
               if (m_pInstance->GetData(TYPE_DECOURER_OF_SOULS) != DONE)
                  {
                   m_pInstance->SetData(TYPE_DECOURER_OF_SOULS, NOT_STARTED);
                   Step = 0;
                   m_creature->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER);
                   m_creature->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
                  } else Step = 10;
   ...
   void UpdateAI(const uint32 diff)
    {
        ...
        if (m_pInstance->GetData(TYPE_DECOURER_OF_SOULS) == DONE)
        {
         ...
         if(m_pInstance->GetData64(DATA_LIDER) == 1 && m_creature->GetEntry() == NPC_SILVANA_END) return;

         if(m_pInstance->GetData64(DATA_LIDER) == 2 && m_creature->GetEntry() == NPC_JAINA_END) return;
             ...
             switch(Step)
               {
                ...
                case 1:
                    if(m_creature->GetEntry() == NPC_JAINA_END)
                    {
                        ...
                        {
                            if (Creature* pTemp = m_creature->SummonCreature(NPC_COLISEUM_CHAMPION_A_M,5623.609f,2457.946f,705.891f,1.37f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000))
                            {
                    ...
                    if(m_creature->GetEntry() == NPC_SILVANA_END)
                    {
                        ...
                        if (uiSummon_counter < 4)
                        {
                            if (Creature* pTemp = m_creature->SummonCreature(NPC_COLISEUM_CHAMPION_H_M,5623.609f,2457.946f,705.891f,1.37f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000))
                            {
                                pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                pTemp->SetUInt64Value(UNIT_FIELD_TARGET, m_creature->GetGUID());
                                switch(uiSummon_counter)
                ...
                case 2:
                    if(m_creature->GetEntry() == NPC_JAINA_END)
                    {
                        ...
                        if (uiSummon_counter < 4)
                        {
                            if (Creature* pTemp = m_creature->SummonCreature(NPC_COLISEUM_CHAMPION_A_F,5623.609f,2457.946f,705.891f,1.37f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000))
                            {
                                pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                pTemp->SetUInt64Value(UNIT_FIELD_TARGET, m_creature->GetGUID());
                                switch(uiSummon_counter)
                    ...
                    if(m_creature->GetEntry() == NPC_SILVANA_END)
                    {
                        ...
                        if (uiSummon_counter < 4)
                        {
                            if (Creature* pTemp = m_creature->SummonCreature(NPC_COLISEUM_CHAMPION_H_F,5623.609f,2457.946f,705.891f,1.37f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000))
                            {
                                pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                pTemp->SetUInt64Value(UNIT_FIELD_TARGET, m_creature->GetGUID());
                                switch(uiSummon_counter)
                ...
                case 3:
                    if(m_creature->GetEntry() == NPC_JAINA_END)
                    {
                        ...
                        if (uiSummon_counter < 4)
                        {
                            if (Creature* pTemp = m_creature->SummonCreature(NPC_COLISEUM_CHAMPION_A_P,5623.609f,2457.946f,705.891f,1.37f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000))
                            {
                                pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                pTemp->SetUInt64Value(UNIT_FIELD_TARGET, m_creature->GetGUID());
                                switch(uiSummon_counter)
                    ...
                    if(m_creature->GetEntry() == NPC_SILVANA_END)
                    {
                        ...
                        if (uiSummon_counter < 4)
                        {
                            if (Creature* pTemp = m_creature->SummonCreature(NPC_COLISEUM_CHAMPION_H_T,5623.609f,2457.946f,705.891f,1.37f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000))
                            {
                                pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                pTemp->SetUInt64Value(UNIT_FIELD_TARGET, m_creature->GetGUID());
                                switch(uiSummon_counter)
