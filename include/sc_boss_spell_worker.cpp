...
{
    if (m_uiSpellIdx > bossSpellCount())
        return;
...
void BossSpellWorker::LoadSpellTable()
{
    ...
    if (Result)
    {
        ...
        do
        {
            ...
            if (bossEntry != bossID)
                error_log("BSW: Unknown error while load boss_spell_table");
            else
                ++uiCount;
    ...
    Unit* pSummon = NULL;
        ...
        switch (pSpell->m_CastTarget) {
                   ...
                   if (pSpell->LocData.z <= 1.0f)
                   {
                       ...
                       if (!pTarget->IsPositionValid() || !pTarget->IsInMap(boss))
                       {
                           if (pTarget->GetTypeId() == TYPEID_PLAYER)
                               ...
                               error_log("BSW: CAST_ON_RANDOM_POINT FAILED: creature has invalid position. SpellID is %u",pSpell->m_uiSpellEntry[currentDifficulty]);
                           return CAST_FAIL_OTHER;
                       }
            ...
            case CAST_ON_RANDOM_PLAYER:
                   if ( pSpell->LocData.x < 1 )
                       pTarget = SelectRandomPlayer();
                   else
                       pTarget = SelectRandomPlayerAtRange((float)pSpell->LocData.x);
                   if (pTarget && pTarget->IsInMap(boss))
                       return _BSWCastOnTarget(pTarget, m_uiSpellIdx);
                   break;
...
CanCastResult BossSpellWorker::_BSWCastOnTarget(Unit* pTarget, uint8 m_uiSpellIdx)
{
    ...
    if (!pSpell->m_IsBugged)
        return _DoCastSpellIfCan(pTarget, pSpell->m_uiSpellEntry[currentDifficulty]);
    else
...
uint8 BossSpellWorker::_findSpellIDX(uint32 SpellID)
{
    if (bossSpellCount() >= 0)
        for(uint8 i = 0; i < bossSpellCount(); ++i)
            if (m_BossSpell[i].m_uiSpellEntry[RAID_DIFFICULTY_10MAN_NORMAL] == SpellID)
                return i;
...
Unit* BossSpellWorker::_doSummon(uint8 m_uiSpellIdx, TempSummonType summontype, uint32 delay)
{
    ...
    if (pSpell->LocData.z <= 1.0f) 
    {
        ...
        return boss->SummonCreature(pSpell->m_uiSpellEntry[currentDifficulty], pSpell->LocData.x, pSpell->LocData.y, pSpell->LocData.z, 0, summontype, delay);
};
...
bool BossSpellWorker::_doAura(uint8 m_uiSpellIdx, Unit* pTarget, SpellEffectIndex index)
{
    ...
    if (_hasAura(m_uiSpellIdx,pTarget))
         ...
         debug_log("BSW: adding new aura from spell %u index %u",pSpell->m_uiSpellEntry[currentDifficulty], index);
...
CanCastResult BossSpellWorker::_CanCastSpell(Unit* pTarget, const SpellEntry *pSpell, bool isTriggered)
{
    ...
    if (const SpellRangeEntry *pSpellRange = GetSpellRangeStore()->LookupEntry(pSpell->rangeIndex))
    {
        if (pTarget != boss)
        {
            if (!pTarget->IsPositionValid() || !boss->IsPositionValid())
                return CAST_FAIL_OTHER;
